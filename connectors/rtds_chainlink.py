...
from config import RTDSConfig, PolymarketGammaConfig, get_config
from models import ChainlinkPriceTick, RTDSSubscription, parse_rtds_message, current_ts_ms
...
class RTDSChainlinkConnector(BaseConnector):
    ...
    def __init__(
        self,
        config: Optional[RTDSConfig] = None,
        gamma_config: Optional[PolymarketGammaConfig] = None
    ):
        """
    ...
    @property
    def last_price(self) -> Optional[ChainlinkPriceTick]:
    ...
    def get_price(self, symbol: str) -> Optional[ChainlinkPriceTick]:
            ...
            if tick:
                ...
                if topic == "crypto_prices_chainlink":
                    if msg_type == "subscribe":
                        self.logger.info(f"Chainlink subscription confirmed")
                    else:
